...
export function SocialDashboardPage() {
  ...
  return <SocialPage title="داشبورد انتشار" actions={<button disabled={busy} onClick={() => void evaluate()}>بررسی فرصت‌های انتشار</button>}>
    ...
    <div className="social-dashboard-grid">
      ...
      <section className="panel"><h2>خط زمانی امروز</h2><div className="social-list">
        {dashboard?.timeline.map((item) => <article className="social-list-row" key={item.id}>
          <span className={`social-dot social-dot-${item.status.toLowerCase()}`} /><div><strong>{item.title}</strong><span>{item.platform ? platformLabel[item.platform] : 'سیستم'} · {item.status}</span></div><time>{dateTime(item.occurredAt)}</time>
        </article>)}
...
export function SocialChannelsPage() {
  ...
  return <SocialPage title="کانال‌ها">
    ...
    <form className="panel social-channel-form" onSubmit={submit}>
      ...
      <div className="actions"><button className="primary" disabled={busy}>ذخیره</button>{editingId && <button type="button" onClick={() => { setEditingId(null); setForm(emptyChannel) }}>انصراف</button>}</div>
    </form>
    <section className="panel table-wrap"><table><thead><tr><th>پلتفرم</th><th>عنوان</th><th>شناسه</th><th>وضعیت اتصال</th><th>آخرین ارسال موفق</th><th>خطای آخر</th><th>عملیات</th></tr></thead><tbody>
      {channels.map((channel) => <tr key={channel.id}><td>{platformLabel[channel.platform]}</td><td>{channel.title}</td><td dir="ltr">{channel.externalChannelId}</td><td><span className={`badge social-connection-${channel.connectionStatus.toLowerCase()}`}>{channel.connectionStatus === 'Connected' ? 'متصل' : channel.connectionStatus === 'Failed' ? 'خطا' : 'بررسی نشده'}</span></td><td>{dateTime(channel.lastSuccessfulPublicationAt)}</td><td className="social-error-cell">{channel.lastPublicationError || '—'}</td><td className="actions"><button onClick={() => edit(channel)}>ویرایش</button><button disabled={busy || !channel.credentialConfigured} onClick={() => void test(channel.id)}>آزمون اتصال</button></td></tr>)}
      {channels.length === 0 && <tr><td colSpan={7}>هنوز کانالی ثبت نشده است.</td></tr>}
    </tbody></table></section>
  </SocialPage>
}
  ...
  return <SocialPage title="انتشار جدید">
    ...
    <div className="social-compose-grid">
      <section className="panel social-compose-form"><h2>ساخت پیش‌نویس</h2>
        ...
        <button type="button" className="secondary" disabled={busy || (templateType !== 'DailyMenu' && templateType !== 'Custom' && !sourceId)} onClick={() => void generate()}>تولید پیش‌نویس از اطلاعات واقعی</button>
      ...
      <section className="panel social-target-form"><h2>مقصد و نسخه پلتفرم</h2>
        ...
        <div className="actions"><button type="button" disabled={!draft || selectedChannels.length === 0 || busy} onClick={() => void preview()}>پیش‌نمایش نهایی</button><button type="button" className="primary" disabled={previews.length === 0 || busy} onClick={() => void publish()}>انتشار</button></div>
      </section>
    </div>
...
export function SocialTemplatesPage() {
  ...
  useEffect(() => { void load() }, [load])
  const save = async (template: SocialTemplateDto) => {
  ...
  return <SocialPage title="قالب‌های پیام"><Feedback error={error} />
    <div className="social-template-grid">{templates.map((template) => <article className="panel social-template-card" key={template.id}>
      ...
      <button className="primary" onClick={() => void save(template)}>ذخیره قالب</button>
    </article>)}</div>
  </SocialPage>
}
...
export function SocialRulesPage() {
  ...
  const saveRule = async (event: FormEvent) => { event.preventDefault(); try { await adminApi.saveSocialRule(editingId, form); setEditingId(null); setForm(defaultRule(channels)); await load() } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) } }
  const saveSettings = async () => { if (!settings) return; try { await adminApi.saveSocialSettings(settings); await load() } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) } }
  return <SocialPage title="قوانین خودکارسازی"><Feedback error={error} />
    ...
    {settings && <section className="panel social-settings-form"><h2>کنترل دفعات انتشار</h2>
      ...
      <label>سکوت از<input type="time" value={settings.quietHoursStart ?? ''} onChange={(event) => setSettings({ ...settings, quietHoursStart: event.target.value || null })} /></label>
      <label>سکوت تا<input type="time" value={settings.quietHoursEnd ?? ''} onChange={(event) => setSettings({ ...settings, quietHoursEnd: event.target.value || null })} /></label>
      ...
      <button className="secondary" onClick={() => void saveSettings()}>ذخیره تنظیمات</button>
    </section>}
    <form className="panel social-rule-form" onSubmit={saveRule}><h2>{editingId ? 'ویرایش قانون' : 'قانون جدید'}</h2>
      ...
      <label>از ساعت<input type="time" value={form.startTime ?? ''} onChange={(event) => setForm({ ...form, startTime: event.target.value || null })} /></label><label>تا ساعت<input type="time" value={form.endTime ?? ''} onChange={(event) => setForm({ ...form, endTime: event.target.value || null })} /></label>
      ...
      <div className="actions"><button className="primary">ذخیره قانون</button>{editingId && <button type="button" onClick={() => { setEditingId(null); setForm(defaultRule(channels)) }}>انصراف</button>}</div>
    </form>
    <section className="panel table-wrap"><table><thead><tr><th>اولویت</th><th>قانون</th><th>نوع</th><th>حالت</th><th>بازه</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>{rules.map((rule) => <tr key={rule.id}><td>{formatNumber(rule.priority)}</td><td>{rule.title}</td><td>{templateLabel[rule.templateType]}</td><td>{modeLabel[rule.executionMode]}</td><td dir="ltr">{rule.startTime || '—'} - {rule.endTime || '—'}</td><td>{rule.isEnabled ? 'فعال' : 'غیرفعال'}</td><td><button onClick={() => edit(rule)}>ویرایش</button></td></tr>)}</tbody></table></section>
  </SocialPage>
}
...
export function SocialSuggestionsPage() {
  ...
  const showPreview = async (suggestion: SocialSuggestionDto) => { try { setPreview({ suggestion, items: await adminApi.previewSocialPost(valueFor(suggestion)) }) } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) } }
  const publish = async () => { if (!preview || !window.confirm('این پیشنهاد منتشر شود؟')) return; try { const post = await adminApi.createSocialPost(valueFor(preview.suggestion)); await adminApi.publishSocialPost(post.id); setPreview(null); await load() } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) } }
  const dismiss = async (id: number) => { try { await adminApi.dismissSocialSuggestion(id); await load() } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) } }
  return <SocialPage title="پیشنهادهای انتشار"><Feedback error={error} />
    <div className="social-suggestion-grid">{items.map((item) => <article className={`panel social-suggestion status-${item.status.toLowerCase()}`} key={item.id}><header><span>{templateLabel[item.templateType]}</span><time>{dateTime(item.createdAt)}</time></header><h2>{item.sourceTitle || item.draft.title}</h2><p>{item.reason}</p><div className="actions">{item.status === 'Pending' && <><button className="danger" onClick={() => void dismiss(item.id)}>رد کردن</button><button className="primary" onClick={() => void showPreview(item)}>پیش‌نمایش</button></>}<span>{item.status === 'Dismissed' ? 'رد شده' : item.status === 'Published' ? 'منتشر شده' : ''}</span></div></article>)}
      {items.length === 0 && <div className="panel social-empty">پیشنهادی برای امروز وجود ندارد.</div>}
    </div>
    {preview && <div className="social-preview-dialog" role="dialog" aria-modal="true"><div className="social-preview-dialog-card"><header><h2>پیش‌نمایش مشتری</h2><button onClick={() => setPreview(null)}>بستن</button></header><PlatformPreview items={preview.items} /><button className="primary" onClick={() => void publish()}>تأیید و انتشار</button></div></div>}
  </SocialPage>
}

export function SocialHistoryPage() {
  const [query, setQuery] = useState<SocialHistoryQuery>({ page: 1, pageSize: 25 })
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); void load({ ...query, page: 1 }) }
  const retry = async (id: number) => { if (!window.confirm('فقط همین مقصد دوباره ارسال شود؟')) return; try { await adminApi.retrySocialTarget(id); await load() } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) } }
  return <SocialPage title="تاریخچه ارسال"><Feedback error={error} />
    <form className="toolbar social-history-filters" onSubmit={submit}><label>از<input type="date" value={query.from ?? ''} onChange={(event) => setQuery({ ...query, from: event.target.value || null })} /></label><label>تا<input type="date" value={query.to ?? ''} onChange={(event) => setQuery({ ...query, to: event.target.value || null })} /></label><label>پلتفرم<select value={query.platform ?? ''} onChange={(event) => setQuery({ ...query, platform: event.target.value as SocialPlatform || null })}><option value="">همه</option>{Object.entries(platformLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>کانال<select value={query.channelId ?? ''} onChange={(event) => setQuery({ ...query, channelId: event.target.value ? Number(event.target.value) : null })}><option value="">همه</option>{channels.map((channel) => <option value={channel.id} key={channel.id}>{channel.title}</option>)}</select></label><label>نوع<select value={query.templateType ?? ''} onChange={(event) => setQuery({ ...query, templateType: event.target.value as SocialPostTemplateType || null })}><option value="">همه</option>{Object.entries(templateLabel).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label>نتیجه<select value={query.status ?? ''} onChange={(event) => setQuery({ ...query, status: event.target.value as SocialTargetStatus || null })}><option value="">همه</option><option value="Pending">در انتظار</option><option value="Publishing">در حال ارسال</option><option value="Published">موفق</option><option value="Failed">ناموفق</option><option value="Unknown">نامشخص</option></select></label><label>مبدأ<select value={query.origin ?? ''} onChange={(event) => setQuery({ ...query, origin: event.target.value as SocialHistoryQuery['origin'] || null })}><option value="">همه</option><option value="Manual">دستی</option><option value="Suggestion">پیشنهاد</option><option value="Automation">خودکار</option></select></label><label>منبع<input value={query.source ?? ''} onChange={(event) => setQuery({ ...query, source: event.target.value || null })} placeholder="نام غذا یا منبع" /></label><button className="primary">جستجو</button></form>
    <section className="panel table-wrap"><table><thead><tr><th>پست</th><th>نوع</th><th>منبع</th><th>زمان ایجاد</th><th>مقصدها</th><th>نتیجه</th><th>خطا/تلاش مجدد</th></tr></thead><tbody>{data?.items.map((post) => <tr key={post.id}><td className="social-post-title"><strong>{post.title || 'بدون عنوان'}</strong><small>{post.defaultText.slice(0, 100)}</small></td><td>{templateLabel[post.templateType]}</td><td>{post.origin}</td><td>{dateTime(post.createdAt)}</td><td>{post.targets.map((target) => <span className="social-target-chip" key={target.id}>{platformLabel[target.platform]} · {target.channelTitle}</span>)}</td><td>{post.status}</td><td>{post.targets.filter((target) => ['Failed', 'Unknown'].includes(target.status)).map((target) => <div className="social-retry" key={target.id}><span>{target.lastError || 'نتیجه نامشخص'}</span><button onClick={() => void retry(target.id)}>تلاش مجدد همین مقصد</button></div>)}</td></tr>)}{data?.items.length === 0 && <tr><td colSpan={7}>سابقه‌ای با این فیلترها وجود ندارد.</td></tr>}</tbody></table></section>
    {data && data.totalPages > 1 && <div className="pagination"><button disabled={data.page <= 1} onClick={() => { const next = { ...query, page: data.page - 1 }; setQuery(next); void load(next) }}>قبلی</button><span>صفحه {formatNumber(data.page)} از {formatNumber(data.totalPages)}</span><button disabled={data.page >= data.totalPages} onClick={() => { const next = { ...query, page: data.page + 1 }; setQuery(next); void load(next) }}>بعدی</button></div>}
  </SocialPage>
}
