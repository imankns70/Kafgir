...
export function MenuPage({ menu, isLoading, error, cartItems, onRetry, onAdd, onQuantityChange }: Props) {
  ...
  const filteredItems = effectiveCategory === 'all'
    ? availableItems
    : availableItems.filter((item) => item.category.slug === effectiveCategory)
  const cartQuantities = useMemo(
    () => new Map(cartItems.map((item) => [item.dailyMenuItemId, item.quantity])),
    [cartItems],
  )

  if (isLoading) return <BrandedState animated title="در حال چیدن سفره امروز…" message="چند لحظه صبر کنید تا غذاهای تازه را بیاوریم." />
  ...
  return <main>
    <section className="menu-intro">
      <HeroCarousel images={availableItems.map((item) => item.imageUrl)} />
